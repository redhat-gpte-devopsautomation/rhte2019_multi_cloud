...
. Set the `SANDBOX_NUMBER` variable to the number of your sandbox and save it in `.bashrc`:
+
[source,sh]
----
...
. After you have the variables set properly, set up two variables for the two clusters and save them in `.bashrc`:
+
[source,sh]
----
...
. Start by creating a federated project. First, create a project on your first cluster:
+
[source,sh]
----
...
----
+
You could have also created the `FederatedNamespace` from a YAML definition. That way, you would not have needed to first create the project and then federate it. In the next few steps, you use the YAML approach. Using `kubefedctl federate` is a convenient way to federate resources that already exist.
